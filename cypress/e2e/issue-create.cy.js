...
describe('Issue create', () => {
	...
	it('Should create an issue with priority and validate it successfully', () => {
		...
		cy.get('[data-testid="modal:issue-create"]').within(() => {
			...
			//open issue type dropdown and choose Story
			cy.get('[data-testid="select:type"]').click();
			cy.get(`[data-testid="select-option:${issueData.type}"]`).trigger(
				'click'
			);
			...
			cy.get(
				`[data-testid="select-option:${issueData.reporter}"]`
			).click();

			//Select user from assignee dropdown
			cy.get('[data-testid="select:userIds"]').click();
			cy.get(
				`[data-testid="select-option:${issueData.assignee}"]`
			).click();

			cy.get('[data-testid="select:priority"]').click();
			cy.get(
				`[data-testid="select-option:${issueData.priority}"]`
			).click();
		...
		cy.get('[data-testid="board-list:backlog')
			...
			.within(() => {
				...
				//Assert that correct avatar and type icon are visible
				cy.get(`[data-testid="avatar:${issueData.reporter}"]`).should(
					'be.visible'
				);
				cy.get(
					`[data-testid="icon:${issueData.type.toLowerCase()}"]`
				).should('be.visible');
			});
	});
  ...
  // P.S Looks like it will fail, since app doesn't trim title 
	it.only('Verifies that application is removing unnecessary spaces on the board view', () => {
		...
		IssueModal.getIssueModal()
			.within(() => {
				// fix for title field being unfocused during typing.
				cy.wait(500);
				IssueModal.editTitle(issueTitle);
